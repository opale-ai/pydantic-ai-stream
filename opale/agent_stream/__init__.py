...
from .stream import is_live as stream_is_live
from .stream import start as stream_start
from .stream import stop as stream_stop

logger = logging.getLogger(__name__)


class AgxCanceledException(Exception):
    pass
    ...
    try:
        async with agent.iter(
            ...
            async for node in agent_run:
                if not await stream_is_live(
                    deps.get_scope_id(), deps.user_id, deps.session_id
                ):
                    raise AgxCanceledException()
        ...
        await stream_stop(deps.get_scope_id(), deps.user_id, deps.session_id)
